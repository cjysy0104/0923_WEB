...
<!-- 
	...
	식별자 - 고유한 이름( ex: PK) -> 실제 위치가 아닌 논리적인 식별값
...
<body>
	...
	<p>
		Jakarta Server Page(JSP) Standard Tag Library의 약어 <br>
	...
	<h5>변수(실제론 속성 == Attribute) 선언하기</h5>
	
	<pre>
		이전에 학습한 Scope들에 새로운 속성 추가하는 방법임.
	...
	value출력은 그냥 써도 댐 : ${ num1 } <br>
	
	<br>
	
	requestScope result : ${ requestScope.result } <br>
	out태그를 사용해보자 : <c:out value="${ requestScope.result }" default="값이 없음"></c:out>
	...
	<!-- choose 태그 안에 when과 otherwise외에 어떤것도 포함되선 안됨(주석포함) -->
	<c:choose>
	...
	<pre>
		...
		&lt;c:forTokens var="값을보관할속성명" items="분리하고자하는문자열" delsmins="구분자">
		
			반복적으로 실행할 문구(출력만, 다른 문구는 오류발생가능성 여지 O)
			
		&lt;/c:forTokens>
		
		-JAVA의 StringTokenizer와 비슷한 역할
		-구분자를 통해 분리된 각각의 문자열에 순차적으로 접근하면서 반수앻
	</pre>
	
	<!-- 테스트할 값 
	<c:set var="device" value: "컴퓨터, 핸드폰, tv,에어컨.냉장고-세탁기"></c:set>
