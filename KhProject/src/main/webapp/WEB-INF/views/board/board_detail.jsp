	...
	<div class="outer">
		...
		<div class="row">
		  <div class="offset-lg-2 col-lg-8">
		    <div class="card">
		      <div class="card-header text-white" style="background-color: #52b1ff;">${ map.board.boardNo}</div>
		      <div class="card-body"> 
		          ...
		          <div class="form-group">
					...
					<c:choose>
						<c:when test="${map.at ne null}">
			            	...
			            	<img src="${ map.at.filePath }/${ map.at.changeName}" width="100" height="100"/>
						</c:when>
						<c:otherwise>
			            	<!-- 첨부파일은 없을수도있음 -->
						...
						<c:choose>
							<c:when test="${ sessionScope.userInfo ne null}">
								<td><textarea id="replyContent" cols="50" rows="3"
										style="resize: none;" class="form-control"></textarea></td>
								<td><button onclick="insertReply();" class="btn"
										style="width: 100%; height: 100%; background-color: #52b1ff; color: white;">댓글등록</button></td>
							</c:when>
							<c:otherwise>
								<td><textarea readonly cols="50" rows="3"
										style="resize: none;" class="form-control">로그인 후 이용가능한 서비스입니다.</textarea>
								</td>
								<td><button class="btn"
										style="width: 100%; height: 100%; background-color: #52b1ff; color: white;">댓글등록</button></td>
							</c:otherwise>
